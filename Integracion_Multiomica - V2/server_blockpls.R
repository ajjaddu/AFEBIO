...
Blockpls <- reactive({
  ...
  if (is.null(input$type_analysis4)) return(NULL)
  if (input$type_analysis4 == "None") return(NULL)
  ...
  omica_seleccionada <- input$predictive_omic
  omica_seleccionada2 <- which(names(lista_omicas)==omica_seleccionada)
  
  if (input$number_omics >= 3 & input$type_analysis4 == "block.PLS"){
    
    design = matrix(0.1, ncol = length(X), nrow = length(X), 
                    dimnames = list(names(X), names(X)))
    diag(design) = 0 # set diagonal to 0s
    
    final.diablo.model = block.pls(X, indY = omica_seleccionada2, ncomp = input$components,design = design,scale = T)
  }
  
  if (input$number_omics >= 3 & input$type_analysis4 == "block.spls"){
    ...
    final.diablo.model = block.spls(X = X, indY = omica_seleccionada2, ncomp = input$components,design = design, keepX=list.keepX, 
                                     scale = T)
  }
